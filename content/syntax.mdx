...
Built-in types that support extension include: `bool`, `string`, `int`, `i8`, `i16`, `i32`, `i64`, `uint`, `u8`, `u16`, `u32`, `u64`, `float`, `f32`, `f64`, `chan`, `vec`, `map`, `set`, `tuple`.
...
`map` is nature's built-in associative array type for storing key-value pairs and stored on the heap.
...
```nature
[u8;3] array = [1, 2, 3] // Declare an array of length 3 with elements of type u8
array[0] = 12
array[1] = 24
var a = array[7]
```
...
var result = match value {
    is int -> 0.0
    is float -> value // value is automatically asserted to float type: var result = value as float
    _ -> 0.0
}
```
...
// 2. There should not be a space between the return parameters and ')'
