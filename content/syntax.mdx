...
if foo is int v {
	// auto as: var v = foo as int
    println("foo is an integer", v + 1)
}
...
int result = match foo {
    is int v -> v        // auto: var v = foo as int
    is null -> -1 
}
```
...
fn use_com(combination c):int {  
    if c is square s {  
        // auto as: var s = c as square, c is interface  
        s.unique()  
        return 1  
    }  
    if c is ptr<square> s {  
        // auto as: var s = c as ptr<square>  
        s.unique()  
        return 2  
    }  
    return 0  
...
fn use(testable? test) {  
    if (test is testable v) { // var v = test as testable  
        println('testable value is', v.nice())  
    } else {  
...
For union types, you can use `is` for type matching, and variables can be added after the type for automatic derivation.
```nature
any value = 2.33
var result = match value {
    is int -> 0.0
    is float v -> v  // auto as: var v = value as float
    _ -> 0.0
}
```
